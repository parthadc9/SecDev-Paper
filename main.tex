...
%Joseph keeping the pop science for now to see how it fits in however I do fully understand your observation.

%\section{Conclusion and future Work}

%Our study explores the relationships between the technical challenges developers face, the behaviors they use to help themselves implement security and the interventions required to help developers program securely.  We find that not all behaviors are due to developers technical struggles, and that not all challenges are met by the interventions currently identified in the literature. There is a worrying trend in the interventions we noted to assume that developers do understand security and that when their failings are pointed out to them, they will understand how to secure their code.  These tropes depend on blaming and coercion. The insulated assumptions are self defeating and does not appear to be correct. 
...
%To search for literature related to developer challenges and interventions we used the following search term on IEEE Explore and the ACM Digital Library, as well as a snowballing method on identified literature.

%\begin{quote}\ttfamily
 %   [[[All: "full text only"] OR [All: :security]] AND [[All: "full text only"] OR [All: :api]]] OR [[[All: "full text only"] OR [All: :library]] AND [[All: "full text only"] OR [All: :interview]]] OR [All: "full text only"] OR [All: :survey] OR [All: "full text only"] OR [All: :questionnaire] OR [[[All: "full text only"] OR [All: :studies]] AND [[All: "full text only"] OR [All: :usability]] AND [[All: "full text only"] OR [All: :developer]] AND [[All: [publication] OR [All: ]]]]
%\end{quote}

%We also manually reviewed several years of the proceedings from the following venues:
%\begin{itemize}
     %\item IEEE Security \& Privacy,
     %\item IEEE Transactions on Software Engineering,
     %\item IEEE Secure Development Conference
     %\item SIGSAC conference on Computer and Communications Security,
     %\item Annual Computer Security Applications Conference,
     %\item New Security Paradigms Workshop,
     %\item International Conference on Software Engineering. 
%\end{itemize}
%Other venues were also picked up as part of the snowballing process.
\end{document}
